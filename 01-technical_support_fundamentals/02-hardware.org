...
CPU also uses cache. Cache is smaller than RAM, but it let's us store data that we use often.
...
There're ways to increase the clock speed of the CPU, called over-clocking. It increases the rate of your CPU clock cycles in order to perform more tasks.
...
+ ATA -- the most common ATA is serial ATA or SATA
  - SATA drive are hot swapable, meaning you don't need turn off your computer to swap them
